...
\section{Class structure}
    ...
    %
    %     \inlinecode{b.get_vertex(i)},
    %     \inlinecode{b.set_vertex(i, v)},
    %     \inlinecode{b.generate_lookup()},
    %     \inlinecode{b.interpolate(t)}, \\
    %     \inlinecode{b.tangent(t)},
    %     \inlinecode{b.second_derivative(t)},
    %     \inlinecode{b.curvature(t)},
    %     \inlinecode{b.get_t_at_distance(d)}, \\
    %     \inlinecode{b.get_point_at_distance(d)},
    %     \inlinecode{b.get_tangent_at_distance(d)},
    %     \inlinecode{b.get_curvature_at_distance(d)}
    %
    %     \paragraph{State Machine -}
    %
    %     This class represents a finite state machine model which utilises function callbacks, it will store a list of transitions and callbacks. When the \inlinecode{transition} method is called the state will change based on the appropriate transition and the callback assigned to it will be executed.
    %
    ...
    %
    %     \inlinecode{s.push(e)}, \inlinecode{s.pop()}, \inlinecode{s.is_empty()}, \inlinecode{s.clear()}
...
\section{Model}
    ...
    \subsection{Classes}
        ...
        \subsubsection{Model}
            ...
            \umldiagram{Model}{
                \inlinecode{- roadNetwork: RoadNetwork}\\
                \inlinecode{- simulation: Simulation}
            }{
                \inlinecode{+ copy\_road\_network(): RoadNetwork}
            }
        ...
        \subsubsection{RoadNetwork}
            ...
            \umldiagram{RoadNetwork}{}{
                \inlinecode{+ get\_bezier(number, number): CubicBezier}
            }
        ...
        \subsubsection{CubicBezier}
            ...
            \umldiagram{CubicBezier}{
                \inlinecode{- vertices: Vector2[]}\\
                \inlinecode{- distanceLookup: Map<number, number>}
            }{
                \inlinecode{+ get_vertex(number): Vector2}\\
                \inlinecode{+ set_vertex(number, Vector2): void}\\
                \inlinecode{- interpolate(number): Vector2}\\
                \inlinecode{- tangent(number): Vector2}\\
                \inlinecode{- second_derivative(number): Vector2}\\
                \inlinecode{- generate_lookup(): void}\\
                \inlinecode{+ get_t_at_distance(number): number}\\
                \inlinecode{+ get_point_at_distance(number): Vector2}\\
                \inlinecode{+ get_tangent_at_distance(number): Vector2}\\
                \inlinecode{+ get_curvature_at_distance(number): number}
            }
...
\section{Network Builder}
    ...
    This behaviour can be modelled by a finite state machine as shown in \autoref{network-builder-state-machine}, this will be implemented using the \inlinecode{StateMachine} utility class and encapsulated by the Controller component.
